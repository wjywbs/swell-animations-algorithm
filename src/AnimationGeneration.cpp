...
void applyRotationPoints(ModelData* modelData, Animation* animation) {
	...
	for (int i = 0; i < modelData->rotationpoints_size(); i++) {
		...
		for (int j = startframe; j <= endframe; j++) {
			animation->mutable_frames(j)->mutable_eularangles()->set_x((1 + i) * rotationAngles.at(i)->x);
			animation->mutable_frames(j)->mutable_eularangles()->set_y((1 + i) * rotationAngles.at(i)->y);
			animation->mutable_frames(j)->mutable_eularangles()->set_z((1 + i) * rotationAngles.at(i)->z);
		}
	}
}
...
void adjustRotationPointLocations(ModelData* modelData, vector<struct pt*> spline) {
	for (int x = 0; x < modelData->rotationpoints_size(); x++) {
		...
		Vector* location = modelData->mutable_controlpoints(startFrame);
		...
		rotPoint->set_startframe(closestFrame);
		rotPoint->set_numframes(min(closestFrame - numFrames, 0));
	}
}
